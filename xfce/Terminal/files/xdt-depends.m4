...
dnl# executed. If this parameter is NOT specified, a diagnostic
...
dnl#
dnl# This macro automatically adds a commandline switch based on the
dnl# "optionname" parameter (--enable-optionname/--disable-optionname),
dnl# which allows the user to explicitly control whether this optional
dnl# dependency should be enabled or not. The "helpstring" parameter gives
dnl# a brief(!) description about this dependency.
dnl#
...
dnl# to the string "yes", in addition to the 4 variables set by
dnl# XDT_CHECK_PACKAGE. But "varname"_FOUND will not be marked for
dnl# substition. Furthermore, a CPP define HAVE_"varname" will be placed in
dnl# config.h (or added to the cc command line, depending on your
dnl# configure.ac) and set to 1.
dnl#
...
dnl#
dnl# Simple wrapper for XDT_CHECK_PACKAGE(["varname"],["package"],["version"]).
dnl# Kept for backward compatibility. Will be removed in the future.
dnl#
AC_DEFUN([BM_DEPEND],
[
  XDT_CHECK_PACKAGE([$1],[$2],[$3])
...
dnl#
dnl# Similar to XDT_CHECK_LIBXPM([]), but fails if the Xpm library is NOT
